...
struct cache_sizes malloc_sizes[] = {//包含26个高速缓存描述符
...
void __init kmem_cache_init(void)
{
	...
	list_add(&cache_cache.next, &cache_chain);//cache_cache是第一个高速缓存的描述符。
	...
	sizes = malloc_sizes;//普通用途的内存区，包括13哥几何分布的内存区。指向26个高速缓存描述符。
	names = cache_names;

	while (sizes->cs_size) {//遍历这些几何分布的内存区。最后一个的大小为0，停止。
		...
		sizes->cs_cachep = kmem_cache_create(names->name,
			...
			(ARCH_KMALLOC_FLAGS | SLAB_PANIC), NULL, NULL);//用于常规分配
	...
	void (*dtor)(void*, kmem_cache_t *, unsigned long))
{
	...
	cachep = (kmem_cache_t *) kmem_cache_alloc(&cache_cache, SLAB_KERNEL);//从第一个普通高速缓存cache_cache里面分配一个告诉缓存描述符。
	if (!cachep)
		goto opps;
...
static inline void * __cache_alloc (kmem_cache_t *cachep, int flags)
{
	...
	ac = ac_data(cachep);//得到本CPU的空闲对象本地告诉缓存。
	if (likely(ac->avail)) {
